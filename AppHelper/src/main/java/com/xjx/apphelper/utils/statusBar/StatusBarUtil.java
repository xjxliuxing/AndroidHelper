package com.xjx.apphelper.utils.statusBar;
...
import com.xjx.apphelper.global.CommonConstant;
...
public class StatusBarUtil {
    ...
    /**
     * @param view
     * @return 设置状态栏的高度, 一般多用于一个activity中有多个fragment中，配合 全屏透明使用
     */
    public StatusBarUtil setStatusViewHeight(View view) {

        int height = SpUtil.getInt(CommonConstant.STATUS_HEIGHT);
        if (height > 0) {
    ...
    public StatusBarUtil setStatusFontColor(boolean dark) {
        ...
        if (TextUtils.equals(deviceBrand, "Xiaomi")) {
            ...
            StatusbarColorUtils.setStatusBarDarkIcon(mActivity, dark);
        } else {
