...
class CustomTouchUnlockView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    //    val intervalWidth = ConvertUtil.toDp(10f).toInt() // 每个view间隔的宽度
    val intervalWidth = 5 // 每个view间隔的宽度
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas?.let { c ->
            ...
            for (index in 0 until mAlphasList.size) {
                val width = mRadiusList[index]
                val alpha = mAlphasList[index]

                mPaintSpread.alpha = alpha
                c.drawCircle(centerX, centerY, (width + radius), mPaintSpread)

                if (alpha > 0 && (width + radius < maxRadius)) {
                    mRadiusList[index] = width + 2
                    if (alpha - 2 > 0) {
                        mAlphasList[index] = alpha - 2
                    } else {
                        mAlphasList[index] = 1
                    }
                }
...
//                    mAlphasList[index] = 255
            ...
            val interval = ((currentValue / intervalWidth) * intervalWidth)
            LogUtil.e("value:$currentValue  interval:$interval" + "  ssss:" + maxRadius / intervalWidth)

            if (currentValue >= (maxRadius / intervalWidth)) {
                mRadiusList.add(0)
                mAlphasList.add(0)
            }
...
//            for (index in 0 until mAlphasList.size) {
//                val alphas = mAlphasList[index]
//                mPaintSpread.alpha = alphas
//
//                // 获取存储半径的值
//                if (index < mRadiusList.size) {
//                    val width = mRadiusList[index]
//                    // 开始绘制圆形
//                    c.drawCircle(centerX, centerY, radius + width, mPaintSpread)
//
//                    // 每次自增1，用来增加动态增加view的宽度
//                    if (width < maxRadius && alphas > 0) {
//                        mRadiusList[index] = (width + 5)
//                        mAlphasList[index] = if (alphas - 5 > 0) alphas - 5 else 1
//                    }
//                }
//
//                // 动态增加view
